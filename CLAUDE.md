...
[vibe-review skill](https://github.com/tsukiyokai/vibe-review-skill) — 审查逻辑的核心，已独立为npm包（`@tsukiyokai/vibe-review`）。SKILL.md定义审查流程、置信度分级、输出格式模板。`references/`下分层存放编码规范（公司→产品线→项目→个人）。渐进式加载：根据repo类型决定加载哪些规范文件。
