# PR #647: Support symmetric memory for aicpu unflod mode

- 作者: linzhenkang
- 分支: master -> master
- 链接: https://gitcode.com/cann/hcomm-dev/merge_requests/647
- 描述: Support symmetric memory for aicpu unflod mode
Debug.
Debug1
rsv static graph memory
[BUGFIX]fix repeat launch
...

## 变更文件 (42 个, 其中 C/C++ 文件 35 个)

- [modified] build.sh (+1, -1)
- [modified] include/hccl/hccl_comm.h (+41, -0) *
- [modified] include/hccl/hccl_types.h (+5, -0) *
- [modified] src/algorithm/impl/operator/all_gather_operator.cc (+1, -1) *
- [modified] src/algorithm/impl/operator/all_reduce_operator.cc (+1, -1) *
- [modified] src/algorithm/impl/operator/broadcast_operator.cc (+1, -1) *
- [modified] src/algorithm/impl/operator/reduce_scatter_operator.cc (+1, -1) *
...
- [added] src/framework/common/src/hccl_mem_alloc.cc (+138, -0) *
...
- [modified] src/framework/communicator/hccl_comm_host.cc (+1, -0) *
...
- [modified] src/framework/communicator/impl/hccl_communicator_host.cc (+60, -1) *
- [modified] src/framework/communicator/impl/independent_op/data_api/hccl_api_data.cc (+1, -1) *
- [added] src/framework/communicator/impl/symmetric_memory/CMakeLists.txt (+8, -0)
- [added] src/framework/communicator/impl/symmetric_memory/symmetric_memory.cc (+563, -0) *
- [added] src/framework/communicator/impl/symmetric_memory/symmetric_memory.h (+97, -0) *
- [modified] src/framework/device/framework/CMakeLists.txt (+1, -0)
- [modified] src/framework/device/framework/aicpu_communicator.cc (+42, -7) *
...
- [added] src/framework/device/framework/aicpu_symmetric_memory.cc (+32, -0) *
...
- [modified] src/framework/op_base/src/op_base.cc (+29, -0) *
- [modified] src/platform/common/externalinput.cc (+43, -1) *
...
```diff
@@ -330,7 +330,7 @@ HcclResult AllGatherOperator::SelectAlgfor91093(const OpParam& param, std::strin
...
+    } else if ((param.supportZeroCopy || param.supportSymmetricMemory) &&
...
```diff
@@ -654,7 +654,7 @@ HcclResult AllReduceOperator::SelectAlgfor91093(const OpParam& param, std::strin
...
```diff
@@ -221,7 +221,7 @@ HcclResult BroadCastOperator::SelectAlgfor91093(const OpParam& param, std::strin
         algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_NHR;
     } else if (smallCountOptimSingleServer) {
...
```diff
@@ -460,7 +460,7 @@ HcclResult ReduceScatterOperator::SelectAlgfor91093(const OpParam& param, std::s
...
+    } else if ((param.supportZeroCopy || param.supportSymmetricMemory) && isSupportInlineReduce &&    // 不申请scratch ==> 不支持非InlineReduce
         (topoType_ == TopoType::TOPO_TYPE_NP_DOUBLE_RING || param.DataDes.count * unitSize * deviceNumPerAggregation_ > HCCL_MID_COUNT_16_MB)) {
         const u32 SEVER_NUM_FOUR = 4;
         constexpr u64 RING_EXCHANGE_PIPELINE_DATA_SIZE_MIN = 2 * 1024 * 1024;
...
+        HCCL_ERROR("[HcclAddVa2Handle] invalid devPtr[%p] handle[%p]",
+        devPtr, handle), HCCL_E_PARA);
+
+    auto it = va2Handle_.find(devPtr);
+    CHK_PRT_RET(it != va2Handle_.end(),
+        HCCL_ERROR("[HcclAddVa2Handle] devPtr[%p] has import", devPtr), HCCL_E_PARA);
+
+    HCCL_INFO("[HcclAddVa2Handle] add devPtr[%p] handle[%p]", devPtr, handle);
+    va2Handle_.insert({devPtr, handle});
+    return HCCL_SUCCESS;
+}
+
+HcclResult aclMemRetainHandle(void *devPtr, void **handle)
+{
+    CHK_PRT_RET((devPtr == nullptr),
+        HCCL_ERROR("[aclMemRetainHandle] invalid devPtr[%p]", devPtr), HCCL_E_PARA);
+
+    auto it = va2Handle_.find(devPtr);
+    CHK_PRT_RET(it == va2Handle_.end(),
+        HCCL_ERROR("[aclMemRetainHandle] devPtr[%p] not import", devPtr), HCCL_E_PARA);
+
+    *handle = va2Handle_[devPtr];
+    HCCL_INFO("[aclMemRetainHandle] get devPtr[%p] handle[%p]", devPtr, *handle);
+
+    return HCCL_SUCCESS;
+}
+
+HcclResult HcclDelVa2Handle(void *devPtr)
+{
+    CHK_PRT_RET((devPtr == nullptr),
+        HCCL_ERROR("[HcclDelVa2Handle] invalid devPtr[%p]", devPtr), HCCL_E_PARA);
+
+    auto it = va2Handle_.find(devPtr);
+    CHK_PRT_RET(it == va2Handle_.end(),
+        HCCL_ERROR("[aclMemRetainHandle] devPtr[%p] not import", devPtr), HCCL_E_PARA);
+
+    void *handle = va2Handle_[devPtr];
+    HCCL_INFO("[aclMemRetainHandle] del devPtr[%p] handle[%p]", devPtr, handle);
+    va2Handle_.erase(it);
+
+    return HCCL_SUCCESS;
+}
...
```diff
@@ -1485,5 +1485,18 @@ HcclResult hcclComm::GetKFCWorkSpace(void **addr, uint64_t *size)
     return HCCL_SUCCESS;
 }
...
### src/framework/communicator/hccl_comm_host.cc
```diff
@@ -253,4 +253,5 @@ namespace hccl
     {
...
### src/framework/communicator/impl/independent_op/data_api/hccl_api_data.cc
```diff
@@ -213,7 +213,7 @@ int32_t HcommReadOnThread(
...
+int32_t HcommNotifyWaitOnThread(ThreadHandle thread, ChannelHandle channel, void *dst, const void *src, uint64_t count,
...
+#include "hccl_comm.h"
+
+extern "C" HcclResult aclMemRetainHandle(void *devPtr, void **handle);
+
+namespace hccl 
+{
+
...
+    
+    // 检查 ptr 是否已经是 granularity 对齐的
+    // 如果 ptr 没有对齐到 granularity，我们其实无法简单通过 Reserve 获得一个 mapped address
+    // 因为 Reserve 得到的是 offset，而 aclMapMem 需要 offset 和 base 对齐。
+    // 这里我们假设 RegisterInternal 能够处理，或者 ptr 本身就是 aligned 的。
+    void* alignedPtr = (void*)((uintptr_t)ptr & ~(granularity_ - 1));
+    
+    // 计算由于指针未对齐产生的额外开销 (虽然你下面用了 alignedPtr 去 retain)
+    size_t ptrDiff = (uintptr_t)ptr - (uintptr_t)alignedPtr;
+    if (ptrDiff > 0) {
+        // 如果 ptr 不是 2M 对齐，alignedSize 可能需要更大来覆盖头部
+        // 但目前的逻辑是 alignedSize 基于 size 向上取整，可能不够严谨，这里先打日志
+        HCCL_WARNING("[SymmetricMemory] User ptr is not aligned to granularity! Diff: %zu", ptrDiff);
+    }
+
+    aclrtDrvMemHandle paHandle;
+    if (aclMemRetainHandle(alignedPtr, &paHandle) != HCCL_SUCCESS) {
+        HCCL_ERROR("[SymmetricMemory][RegisterSymmetricMem] aclMemAttainHandle failed for ptr[%p], size[%ld]. ", ptr, size);
+        return HCCL_E_PARA;
+    }
+
+    HCCL_INFO("[SymmetricMemory][RegisterSymmetricMem] Retained paHandle[%llu] for ptr[%p], alignedSize[%zu]. Total Stride: %zu",
+        paHandle, ptr, alignedSize, stride_);
+
+    size_t offset = 0;
...
+                   alignedSize, alignedSize, granularity_, stride_);
+        return HCCL_E_MEMORY;
+    }
+
...
+    if (ret != HCCL_SUCCESS) {
+        HCCL_ERROR("[SymmetricMemory] AddSymmetricWindow Failed! Releasing offset 0x%zx", offset);
+        (void)vaAllocator_->Release(offset, alignedSize);
+        return ret;
+    }
+    *devWin = pWin->devWin;
...
+        ACL_RT_VMM_EXPORT_FLAG_DISABLE_PID_VALIDATION, &shareableHandle) != ACL_SUCCESS) {
+        HCCL_ERROR("[SymmetricMemory][RegisterInternal] Failed to export shareable handle. offset: %zu, size: %zu",
+            offset, mapSize);
+        return HCCL_E_DRV;
+    }
+
+    HCCL_INFO("[SymmetricMemory][RegisterInternal] Local shareable handle: 0x%016llx, offset: %zu, size: %zu",
+        shareableHandle, offset, mapSize);
+
+    if (ExchangePhyAddrHandle((void*)&shareableHandle, (void*)remoteShareableHandles.data(), 1,
+        HCCL_DATA_TYPE_UINT64) != HCCL_SUCCESS) {
+        return HCCL_E_INTERNAL;
+    }
+
+    HCCL_INFO("[SymmetricMemory][RegisterInternal] remote shareable handle: 0x%016llx, 0x%016llx", remoteShareableHandles[0], remoteShareableHandles[1]);
+
+    u32 i = 0;
...
+        } else if (aclrtMemImportFromShareableHandle(remoteShareableHandles[i], devicePhyId_, 
...
+    strcpy_s(config.hcclCommName, sizeof(config.hcclCommName), "symmetric_memory_sub_comm");
...
+    CHK_RET(hcclStreamSynchronize(stream_->ptr(), 120));
...
@@ -313,7 +314,31 @@ HcclResult HcclCommAicpu::PrepareUserMemRanges(const OpParam &param, const AlgRe
...
@@ -633,6 +658,11 @@ void HcclCommAicpu::SetZeroCopyEnable(bool enable)
     isZeroCopy_ = enable;
 }
...
+
 ...
 {
@@ -2196,12 +2226,17 @@ HcclResult HcclCommAicpu::ExecOp(const std::string &newTag, const std::string &a
...
```diff
@@ -400,6 +400,10 @@ HcclResult AicpuHcclProcess::AicpuRunRpcServerV2(
...
@@ -409,6 +413,7 @@ HcclResult AicpuHcclProcess::AicpuRunRpcServerV2(
...
+    void *peerVa = reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(symWin->userVa) + peerOffset);
+    return peerVa;
+}
+
+}
...
```diff
@@ -1357,6 +1357,7 @@ HcclResult InitCommRootInfo(const u32 nRanks, const u32 rank, const HcclRootHand
...
@@ -4413,6 +4414,34 @@ HcclResult CommGetCCLBufSizeCfg(HcclComm comm, uint64_t *cclBufSize)
...
```diff
@@ -498,6 +498,20 @@ HcclResult InitEnvVarParam()
             ret),
         ret);
...
@@ -1291,6 +1305,29 @@ HcclResult ParseRDMARetryCnt()
     return HCCL_SUCCESS;
 }
...
@@ -2091,4 +2128,9 @@ const u64& GetExternalInputDebugConfig()
...
@@ -217,6 +220,8 @@ HcclResult ParseRDMATimeOut(std::pair<u32, u32> &rdmaTimeOutRange);
...
```diff
@@ -2054,6 +2054,32 @@ aclError aclrtFreePhysical(aclrtDrvMemHandle handle)
     return ACL_SUCCESS;
 }
...
> 注: 以下非 C/C++ 文件未纳入审查: build.sh, src/framework/CMakeLists.txt, src/framework/common/src/CMakeLists.txt, src/framework/communicator/impl/CMakeLists.txt, src/framework/communicator/impl/symmetric_memory/CMakeLists.txt, src/framework/device/framework/CMakeLists.txt, test/llt/ut/single_test/impl/CMakeLists.txt
